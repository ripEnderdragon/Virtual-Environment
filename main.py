...
@app.route("/")
def homepage():
    return '<h1>Pilih salah satu link untuk memulai #Tips: refresh atau muat ulang untuk mengacak ulang jawaban</h1> <button type="button"><a href="/lempar_koin">Flip Coin!</a></button> <button type="button"><a href="/random_fact">View a random fact!</a></button> '
...
def lemparkoin():
    return f'<p>{random.choice(coin_c)}</P>'
app.run (debug=True)
